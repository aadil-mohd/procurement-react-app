...
import { createNewUserAsync, updateUserAsync } from '../../../services/userService';
...
const CreateUserForm: React.FC<ICreateUserForm> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedPhone = `${countryCodes?.find((x)=>x.id==userDetail.countryCode)?.code}-${userDetail.phone}`;
      console.log({
        ...userDetail,
      ...
      if (type === "edit") {
        await updateUserAsync(userDetail.id as string, formData);
        notification.success({
          message: "User updated successfully",
          description: `User ${userDetail.name} has been updated.`
        });
      } else {
        await createNewUserAsync(formData);
        notification.success({
